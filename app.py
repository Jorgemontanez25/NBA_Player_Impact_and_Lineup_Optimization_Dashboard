...
def load_data():
    ...
    return summary, players, lineups, pairs, model_results, feature_importance, teams, rosters, recommendations, opponent_profiles
...
# Force pair names from saved name columns
if {"player_1_name", "player_2_name"}.issubset(pairs.columns):
    pairs["pair_name"] = (
        pairs["player_1_name"].astype(str)
        + " + "
        + pairs["player_2_name"].astype(str)
    )
else:
    pairs["pair_name"] = (
        pairs["player_1"].astype(str)
        + " + "
        + pairs["player_2"].astype(str)
    )
...
def create_court_html(home_team, away_team, home_players, away_players):
    home_positions = [("11%", "45%"), ("23%", "24%"), ("23%", "66%"), ("36%", "35%"), ("36%", "58%")]
    away_positions = [("78%", "45%"), ("66%", "24%"), ("66%", "66%"), ("53%", "35%"), ("53%", "58%")]
    ...
    <style>
        ...
        .half-line { position: absolute; left: 50%; top: 0; width: 4px; height: 100%; background: white; opacity: 0.85; }
        .center-circle { position: absolute; left: calc(50% - 55px); top: calc(50% - 55px); width: 110px; height: 110px; border: 4px solid white; border-radius: 50%; opacity: 0.85; }
        .paint-left { position: absolute; left: 0; top: 145px; width: 135px; height: 140px; border: 4px solid white; border-left: none; opacity: 0.85; }
        .paint-right { position: absolute; right: 0; top: 145px; width: 135px; height: 140px; border: 4px solid white; border-right: none; opacity: 0.85; }
        .player-dot {
            position: absolute; width: 130px; min-height: 44px; border-radius: 999px;
            color: white; text-align: center; padding: 8px 10px; font-size: 12px;
            font-weight: 700; border: 2px solid white; display: flex;
            align-items: center; justify-content: center; box-shadow: 0 3px 8px rgba(0,0,0,0.35);
        }
        .home-dot { background: #2563eb; }
        .away-dot { background: #dc2626; }
        .team-label-left { position: absolute; left: 20px; top: 15px; color: white; font-weight: 900; font-size: 18px; text-shadow: 1px 1px 4px black; }
        .team-label-right { position: absolute; right: 20px; top: 15px; color: white; font-weight: 900; font-size: 18px; text-shadow: 1px 1px 4px black; }
    </style>
    </head>
    <body>
    """
    ...
    for player, (left, top) in zip(home_players, home_positions):
        html += f'<div class="player-dot home-dot" style="left:{left}; top:{top};">{player}</div>'

    for player, (left, top) in zip(away_players, away_positions):
        html += f'<div class="player-dot away-dot" style="left:{left}; top:{top};">{player}</div>'
...
with tab0:
    ...
    with col_b:
        selected_opponent = st.selectbox(
            "Opponent Team",
            [team for team in team_names if team != selected_team]
        )

    selected_team_id = teams.loc[teams["team_name"] == selected_team, "team_id"].iloc[0]
    opponent_team_id = teams.loc[teams["team_name"] == selected_opponent, "team_id"].iloc[0]
    ...
    if len(selected_opponent_players) != 5:
        ...
        if team_recommendations.empty:
            st.warning("No historical lineup candidates found for this team.")
        else:
            ...
            recommended = (
                ...
                .sort_values(["matchup_adjusted_score", "total_duration"], ascending=[False, False])
                .head(5)
                .copy()
            )
            ...
            st.dataframe(
                recommended[[col for col in display_cols if col in recommended.columns]],
...
with tab1:
    ...
    st.dataframe(
        filtered_players[[col for col in player_cols if col in filtered_players.columns]]
        .sort_values("impact_per_60_sec", ascending=False),
        use_container_width=True,
        hide_index=True
    )
...
with tab2:
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(filtered_lineups["total_duration"], filtered_lineups["net_per_60"], alpha=0.7)
    ...
    st.dataframe(
        filtered_lineups.sort_values(["net_per_60", "total_duration"], ascending=[False, False]),
        use_container_width=True,
        hide_index=True
    )
...
with tab3:
    ...
    st.caption("Debug preview — if this shows names, the chart should show names too.")
    st.dataframe(
        pairs[["player_1", "player_1_name", "player_2", "player_2_name", "pair_name"]].head(10),
        use_container_width=True,
        hide_index=True
    )
    ...
    st.dataframe(
        filtered_pairs[[col for col in pair_cols if col in filtered_pairs.columns]]
        .sort_values(["impact_per_60", "total_duration"], ascending=[False, False]),
        use_container_width=True,
        hide_index=True
    )
